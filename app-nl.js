...
let state = {
  ...
  profile: { name:'', title:"My Goals, Insha'Allah", photo: null, photoId: null, timezone: 'Europe/Amsterdam', language: 'en', theme: 'orange', customThemeColor: null, accent: 'orange', customBgColor: null, customCardColor: null, customTextColor: null, bannerBgType: 'default', bannerBgColor: '#1a1e24', bannerBgPhotoId: null, bannerBgPosX: 50, bannerBgPosY: 50, bannerBgZoom: 1, bannerBgFlip: false, photoPosX: 50, photoPosY: 50, photoZoom: 1, photoFlip: false },
  profileGoals: [],
...
async function decryptWithKey(raw, key){
  ...
  try{
    ...
    if(blob && blob.enc){
      if(!key) return null; // encrypted, but we don't have a key to open it — never return the raw ciphertext
      const decrypted = await decryptState(blob, key);
      return decrypted.data;
    }
  }catch(e){ /* not JSON — it's already a plain data URL */ }
  return raw;
}
async function idbGetPhotoSecure(id){
  ...
  if(!raw) return null;
  return decryptWithKey(raw, cryptoKey);
}
...
async function reencryptAllPhotos(oldKey, newKey){
  ...
  for(const ref of refs){
    ...
    try{
      ...
      if(!raw) continue;
      const plain = await decryptWithKey(raw, oldKey);
...
async function save(){
  ...
  try{
    if(cryptoKey){
      const blob = await encryptState(state, cryptoKey);
  ...
  if(meta.locked && (meta.salt || meta.keyFileEnabled)){
    ...
    try{
      ...
      if(raw){
        ...
        if(!parsed.enc) state = Object.assign(state, parsed);
      }
    }catch(e){}
...
async function attemptUnlock(){
  ...
  try{
    const key = meta.keyFileEnabled
      ? await unwrapDataKey({iv:pendingLockKeyFile.iv, data:pendingLockKeyFile.data}, await deriveKey(pw, pendingLockKeyFile.salt, pendingLockKeyFile.iterations || LEGACY_KDF_ITERATIONS))
      : await deriveKey(pw, meta.salt, meta.kdfIterations || LEGACY_KDF_ITERATIONS);
    const raw = localStorage.getItem(STORE_KEY);
...
function renderLockCard(){
  ...
  if(meta.locked){
    ...
    row.innerHTML = `
      ...
      <button class="btn btn-danger btn-sm" data-action="removePassword">${t('btn_removepassword')}</button>
    `;
...
async function removePassword(){
  ...
  await reencryptAllPhotos(oldKey, null);
  cryptoKey = null;
  setMeta({ locked:false });
  await save();
  renderLockCard();
  updateLockIndicator();
}
...
async function performImport(){
  ...
  try{
    const key = needsKeyFile
      ? await unwrapDataKey({iv:pendingImportKeyFile.iv, data:pendingImportKeyFile.data}, await deriveKey(pw, pendingImportKeyFile.salt, pendingImportKeyFile.iterations || LEGACY_KDF_ITERATIONS))
      : await deriveKey(pw, pendingImportBlob.salt, pendingImportBlob.iterations || LEGACY_KDF_ITERATIONS);
    ...
    newState.profile.photoId = importedPhotos.profile ? await storePhoto(importedPhotos.profile) : null;
    newState.profile.bannerBgPhotoId = importedPhotos.banner ? await storePhoto(importedPhotos.banner) : null;
    newState.photos = { before:[], after:[] };
    for(const item of (importedPhotos.before || [])){
      if(item.data) newState.photos.before.push({ ref: await storePhoto(item.data), date:item.date });
    }
    for(const item of (importedPhotos.after || [])){
      if(item.data) newState.photos.after.push({ ref: await storePhoto(item.data), date:item.date });
    }
...
const TTS_PHRASES = {
  en: { get_ready:'Get ready', rest:'Rest', do:'Do', reps_word:'reps', complete:'Workout complete. Great job.', round:'Round', next_up:'Next up:', hold_for:s=>`Hold for ${s} seconds`,
    warmup_intro:'Warm up.', warmup_outro:"Tap skip when you're ready to start.", set_of:(s,t)=>`Set ${s} of ${t}` },
  nl: { get_ready:'Klaar maken', rest:'Rust', do:'Doe', reps_word:'herhalingen', complete:'Training voltooid. Goed gedaan.', round:'Ronde', next_up:'Volgende:', hold_for:s=>`Hou ${s} seconden vast`,
    warmup_intro:'Warming-up.', warmup_outro:'Tik op overslaan als je klaar bent om te beginnen.', set_of:(s,t)=>`Set ${s} van ${t}` },
  ar_fusha: { get_ready:'استعد', rest:'راحة', do:'قم بـ', reps_word:'تكرار', complete:'اكتمل التمرين. أحسنت.', round:'الجولة', next_up:'التالي:', hold_for:s=>`حافظ على الوضعية ${s} ثانية`,
    warmup_intro:'الإحماء.', warmup_outro:'اضغط تخطي عندما تكون مستعدًا للبدء.', set_of:(s,t)=>`المجموعة ${s} من ${t}` }
};
...
function applyAccentColor(){
  ...
  let hex;
  if(accentKey === 'custom'){
    hex = state.profile.customThemeColor || '#ff8c42';
  } else {
...
function renderThemeSwatches(){
  ...
  if(fullRow){
    ...
    const customBg = state.profile.customBgColor || '#171d25';
    const customCard = state.profile.customCardColor || '#242e3a';
  ...
  if(panel){
    ...
    if(bgInput) bgInput.value = state.profile.customBgColor || '#171d25';
    if(cardInput) cardInput.value = state.profile.customCardColor || '#1e2630';
    if(textInput) textInput.value = state.profile.customTextColor || '#eef1f5';
  }
  if(row){
...
// ── Banner background (solid colour / uploaded image) ──
async function setBannerBgType(type){
  state.profile.bannerBgType = type;
  await applyBannerBg();
  renderBannerBgControls();
  save();
}
async function setBannerBgColor(hex){
  state.profile.bannerBgColor = hex;
  await applyBannerBg();
  save();
}
async function applyBannerBg(){
  const topbar = document.querySelector('.topbar');
  if(!topbar) return;
  const layer = document.getElementById('topbarBgLayer');
  const type = state.profile.bannerBgType || 'default';
  if(type === 'solid'){
    if(layer){ layer.style.backgroundImage = 'none'; layer.style.transform = 'scale(1)'; }
    topbar.style.backgroundImage = 'none';
    topbar.style.backgroundColor = state.profile.bannerBgColor || '#1a1e24';
  } else if(type === 'image'){
    topbar.style.backgroundImage = 'none';
    topbar.style.backgroundColor = '';
    if(layer){
      const dataUrl = state.profile.bannerBgPhotoId ? await fetchPhoto(state.profile.bannerBgPhotoId) : null;
      layer.style.backgroundImage = dataUrl ? `url(${dataUrl})` : 'none';
      layer.style.backgroundPosition = `${state.profile.bannerBgPosX ?? 50}% ${state.profile.bannerBgPosY ?? 50}%`;
      layer.style.transform = `scaleX(${state.profile.bannerBgFlip ? -1 : 1}) scale(${state.profile.bannerBgZoom ?? 1})`;
    }
  } else {
    // 'default' uses stylesheet rules (gradient) - clear any inline override left over from 'solid'/'image'.
    if(layer){ layer.style.backgroundImage = 'none'; layer.style.transform = 'scale(1)'; }
    topbar.style.backgroundImage = '';
    topbar.style.backgroundColor = '';
  }
}
function renderBannerBgControls(){
  const type = state.profile.bannerBgType || 'default';
  document.querySelectorAll('#bannerBgTypeRow .seg-btn').forEach(b=>{
    b.classList.toggle('active', b.dataset.val === type);
  });
  const solidRow = document.getElementById('bannerBgSolidRow');
  const imageRow = document.getElementById('bannerBgImageRow');
  if(solidRow) solidRow.style.display = type === 'solid' ? 'block' : 'none';
  if(imageRow) imageRow.style.display = type === 'image' ? 'block' : 'none';
  const colorInput = document.getElementById('bannerBgColorInput');
  if(colorInput) colorInput.value = state.profile.bannerBgColor || '#1a1e24';
}
...
function switchToolsView(view){
  if(view === 'website'){
    window.open('https://beerleert.github.io/', '_blank', 'noopener');
    closeAllSubviewMenus();
    return;
  }
...
function deriveStatsFromDetail(detail){
  ...
  const holdMatch = detail.match(/(\d+)\s*(?:s|sec|secs|second|seconds)?\s*hold/i) ||
                     detail.match(/hold\s*(?:for)?\s*(\d+)\s*(?:s|sec|secs|second|seconds)?/i);
  const repsMatch = detail.match(/[×x]\s*([a-z0-9–\-\s]+?)\s*reps?\b/i);
...
function parseProgramCSV(text){
  ...
  for(let i=1;i<dataRows.length;i++){
    ...
    if(!curDy || dayLabel !== lastDay){
      curDy = { label: dayLabel, rest: isRest, warmup: isRest?'':warmup, restNote: isRest?dayNote:'', exercises: [] };
      curBlk.days.push(curDy);
    }
    ...
    const parts = [];
    if(sets) parts.push(sets + ' ' + t('lbl_set_plural'));
...
# a workout program. Please:
#  1. Keep the keywords below EXACTLY as shown — PROGRAM, BLOCK,
#     INFO, DAY, REST, WARMUP, NOTE, and the "- name | detail |
#     tip" exercise line. They are fixed keywords the app's parser
#     looks for, not labels to translate or rephrase.
#  2. Replace the example plan below with the real program, using
#     this same structure.
#  3. Write each exercise's <detail> using EXACTLY the pattern "N
#     sets x N reps, rest Ns" (e.g. "3 sets x 10 reps, rest 45s")
#     for rep-based work, or "N sets x Ns hold, rest Ns" (e.g. "3
#     sets x 30s hold, rest 30s") for anything measured by TIME
#     instead of a rep count — this covers static holds like planks
#     AND timed cardio/circuit intervals like "45 seconds of jumping
#     jacks" or "30 seconds of mountain climbers". If the workout
#     describes an exercise by a duration rather than a rep count,
#     it is a "hold" line, even if nothing is being held still.
...
#
# ------------------------------------------------------------
# FORMAT RULES (for humans editing by hand, too)
# ------------------------------------------------------------
...
#                          put it right after the DAY: line.
#  NOTE: <text>            optional note for a REST day.
#  WARMUP: <text>          optional warm-up for a training day
#                          (skip this for rest days).
#  - name | detail | tip   one exercise. Start the line with "-".
#                          <detail> and <tip> are optional, but keep
#                          both "|" characters even if you skip one,
#                          e.g. "- Plank | 3 sets x 30s hold, rest
#                          30s |"
#
# Blank lines are ignored — use them freely to keep things
# readable. Everything below the "=" line is a working example:
# edit it in place, or delete it and write your own from scratch.
# ============================================================
...
- Pushups | 3 sets x 10 reps, rest 45s | Keep your core tight
- Squats | 3 sets x 15 reps, rest 45s |
- Jumping Jacks | 3 sets x 45s hold, rest 15s | Timed cardio interval — uses "hold" pattern even though nothing is held still
- Plank | 3 sets x 30s hold, rest 30s | Keep hips level, don't sag
...
DAY: Wednesday
WARMUP: 5 min light cardio, arm circles
- Bench Press | 4 sets x 8 reps, rest 90s | Control the negative
- Pull-ups | 4 sets x 6 reps, rest 90s |
...
DAY: Friday
- Deadlifts | 3 sets x 6 reps, rest 120s | Keep your back neutral
- Lunges | 3 sets x 12 reps, rest 45s |
...
DAY: Monday
- Pushups | 4 sets x 12 reps, rest 40s |
`;
}
...
# IF AN AI IS FILLING THIS OUT
# ------------------------------------------------------------
# Someone has likely pasted this file and asked you to write them a workout program. Please:
# 1. Keep the header row exactly as it is (Program, Block, Block Info, Day, Day Type, Warmup, Day Note, Exercise, Sets, Reps, Rest Sec, Hold Sec, Tip) -- those are fixed column names the app looks for, not labels to translate.
# 2. One row equals one exercise. For a rest day, write a single row with Day Type set to Rest and everything from Exercise onward left blank.
...
# 4. Sets is a plain number. Rest Sec and Hold Sec are in seconds. An exercise is either measured by REPS or by TIME, never both:
#    - Rep-based (e.g. "3 sets of 10 reps"): put the number/range in Reps, leave Hold Sec blank.
#    - Time-based (e.g. "45 seconds of jumping jacks", "30 second plank", "AMRAP for 1 minute"): put the seconds in Hold Sec, leave Reps blank.
#    Time-based covers BOTH static holds like planks AND timed cardio/circuit intervals like jumping jacks, mountain climbers, or high knees -- if the workout gives a duration instead of a rep count, it goes in Hold Sec, not Reps.
#    COMMON MISTAKE TO AVOID: never write a duration as text into Reps (e.g. Reps = "45 sec") -- the app only recognizes Hold Sec as timed, so a duration left in Reps is silently treated as rep-based and the countdown/voice coach won't run for it.
...
# Reps        reps per set -- a number or text like 12-20 or AMRAP. For time-based exercises (including timed cardio intervals, not just static holds) leave this blank and use Hold Sec instead.
# Rest Sec    rest between sets, in seconds, e.g. 45.
...
#`.split('\n');
  ...
  rows.push('');
  rows.push(csvRow(['Program','Block','Block Info','Day','Day Type','Warmup','Day Note','Exercise','Sets','Reps','Rest Sec','Hold Sec','Tip']));
  const add = (block, blockInfo, day, dayType, warmup, dayNote, ex, sets, reps, restSec, holdSec, tip) => {
    rows.push(csvRow(['My Custom Plan', block, blockInfo, day, dayType, warmup, dayNote, ex, sets, reps, restSec, holdSec, tip]));
  };
  add('Week 1','Getting started -- focus on form over speed.','Monday','Training','5 min light cardio, arm circles','','Pushups',3,'10',45,'','Keep your core tight');
  add('Week 1','Getting started -- focus on form over speed.','Monday','Training','5 min light cardio, arm circles','','Squats',3,'15',45,'','');
  add('Week 1','Getting started -- focus on form over speed.','Monday','Training','5 min light cardio, arm circles','','Jumping Jacks',3,'',15,45,'Timed cardio interval -- seconds go in Hold Sec, Reps stays blank');
  add('Week 1','Getting started -- focus on form over speed.','Monday','Training','5 min light cardio, arm circles','','Plank',3,'',30,30,"Keep hips level, don't sag");
  add('Week 1','Getting started -- focus on form over speed.','Tuesday','Rest','','Stretch and recover','','','','','','');
  add('Week 1','Getting started -- focus on form over speed.','Wednesday','Training','5 min light cardio, arm circles','','Bench Press',4,'8',90,'','Control the negative');
  add('Week 1','Getting started -- focus on form over speed.','Wednesday','Training','5 min light cardio, arm circles','','Pull-ups',4,'6',90,'','');
  add('Week 1','Getting started -- focus on form over speed.','Thursday','Rest','','Light walk if you feel like it','','','','','','');
  add('Week 1','Getting started -- focus on form over speed.','Friday','Training','','','Deadlifts',3,'6',120,'','Keep your back neutral');
  add('Week 1','Getting started -- focus on form over speed.','Friday','Training','','','Lunges',3,'12',45,'','');
  ...
  add('Week 2','Add a bit more volume.','Monday','Training','','','Pushups',4,'12',40,'','');
  return rows.join('\n');
}
...
function renderDayContent(){
  ...
  html += d.exercises.map((ex,i)=>`
    <div class="ex-card" data-idx="${i}">
      <div class="ex-top">
        <div class="drag-handle" data-drag-group="exercise"></div>
        <div class="ex-main">
          ...
          <div class="ex-structured-row">
            ${(ex.sets||ex.reps||ex.holdSec||ex.restSec) ? `<span class="ex-structured-badge" data-action="openExSetsModal" data-idx="${i}">${[ex.sets?ex.sets+' '+(ex.sets===1?t('lbl_set_singular'):t('lbl_set_plural')):'', ex.mode==='time' ? (ex.holdSec?ex.holdSec+'s '+t('lbl_holdword'):'') : (ex.reps?ex.reps+' '+t('lbl_repsword'):''), ex.restSec?ex.restSec+'s '+t('lbl_restword'):''].filter(Boolean).join(' · ')}</span>` : `<span class="ex-structured-badge muted" data-action="openExSetsModal" data-idx="${i}">${t('badge_nosetsrest')}</span>`}
            <div class="ex-structured-btns">
              <button class="btn btn-ghost btn-sm ex-video-btn" data-action="openExerciseVideoSearch" data-idx="${i}" title="${t('btn_howtovideo')}" aria-label="${t('btn_howtovideo')}">🔍</button>
            </div>
          </div>
        </div>
        <button class="ex-del" data-action="deleteExercise" data-idx="${i}">✕</button>
      </div>
    </div>
  `).join('');
  wrap.innerHTML = html;
...
// ── EXERCISE HOW-TO (video search) ──────────
function openExerciseVideoSearch(i){
  const ex = curDay().exercises[i];
  const name = (ex && ex.name) ? ex.name.trim() : '';
  const q = encodeURIComponent((name || 'exercise') + ' proper form tutorial');
  window.open('https://www.youtube.com/results?search_query=' + q, '_blank', 'noopener');
}

// ── EXERCISE LIBRARY ─────────────────────────────────
const EXERCISE_LIBRARY_EN = {
  'Chest': ['Barbell Bench Press','Incline Dumbbell Press','Decline Bench Press','Push-Ups','Cable Fly','Dips','Machine Chest Press','Pec Deck','Diamond Push-Ups','Wide Push-Ups','Incline Push-Ups','Decline Push-Ups','Archer Push-Ups','Dumbbell Bench Press','Incline Barbell Press','Cable Crossover','Dumbbell Pullover','Floor Press','Plyo Push-Ups'],
  'Back': ['Pull-Ups','Lat Pulldown','Bent-Over Barbell Row','Seated Cable Row','Deadlift','T-Bar Row','Single-Arm Dumbbell Row','Face Pull','Chin-Ups','Inverted Row','Superman','Reverse Snow Angels','Wide-Grip Pull-Ups','Pendlay Row','Straight-Arm Pulldown','Rack Pull','Good Morning','Back Extension'],
  'Legs': ['Barbell Squat','Walking Lunges','Leg Press','Romanian Deadlift','Calf Raise','Leg Extension','Leg Curl','Bulgarian Split Squat','Hip Thrust','Bodyweight Squat','Jump Squat','Reverse Lunge','Wall Sit','Step-Ups','Glute Bridge','Single-Leg Glute Bridge','Calf Raise (Bodyweight)','Front Squat','Goblet Squat','Sumo Deadlift','Pistol Squat','Box Jump','Seated Calf Raise'],
  'Shoulders': ['Overhead Press','Lateral Raise','Front Raise','Arnold Press','Rear Delt Fly','Upright Row','Shrugs','Pike Push-Ups','Handstand Push-Ups','Wall Walk','Dumbbell Shoulder Press','Cable Lateral Raise','Push Press','Band Pull-Apart','Seated Dumbbell Press','Plate Front Raise'],
  'Arms': ['Barbell Bicep Curl','Hammer Curl','Incline Dumbbell Curl','Tricep Dip','Skull Crusher','Cable Tricep Pushdown','Preacher Curl','Chair Dips','Close-Grip Push-Ups','Concentration Curl','Cable Curl','Overhead Tricep Extension','EZ-Bar Curl','Reverse Curl','Diamond Dips'],
  'Core': ['Plank','Crunches','Russian Twist','Hanging Leg Raise','Mountain Climbers','Bicycle Crunch','Side Plank','Ab Wheel Rollout','Flutter Kicks','V-Ups','Dead Bug','Hollow Hold','Leg Raise','Sit-Ups','Reverse Crunch','Pallof Press','Cable Crunch','Woodchopper','Bird Dog'],
  'Cardio': ['Jumping Jacks','Burpees','High Knees','Jump Rope','Sprint Intervals','Mountain Climbers','Rowing Machine','Stair Climber','Burpee Broad Jump','Skater Jumps','Bear Crawl','Butt Kicks','Battle Ropes','Box Step-Ups','Cycling','Jumping Lunges','Tuck Jumps','Shadow Boxing'],
  'Glutes': ['Barbell Hip Thrust','Cable Kickback','Frog Pump','Banded Lateral Walk','Donkey Kick','Fire Hydrant','Single-Leg Deadlift','Clamshell'],
  'Full Body': ['Kettlebell Swing','Thruster','Turkish Get-Up',"Farmer's Carry",'Wall Ball','Clean and Press','Devil Press','Man Maker']
};
const EXERCISE_LIBRARY_NL = {
  ...
  'Rug': ['Pull-Ups','Lat Pulldown','Voorovergebogen Barbell Row','Zittende Kabel Row','Deadlift','T-Bar Row','Eenarmige Halter Row','Face Pull','Chin-Ups','Inverted Row','Superman','Omgekeerde Sneeuwengelen','Brede Pull-Ups','Pendlay Row','Straight-Arm Pulldown','Rack Pull','Good Morning','Rugstrekken'],
  'Benen': ['Barbell Squat','Wandelende Lunges','Leg Press','Romeinse Deadlift','Kuitheffen','Beenstrekken','Beenbuigen','Bulgaarse Split Squat','Hip Thrust','Bodyweight Squat','Jump Squat','Achterwaartse Lunge','Wall Sit','Step-Ups','Glute Bridge','Eenbenige Glute Bridge','Kuitheffen (Lichaamsgewicht)','Front Squat','Goblet Squat','Sumo Deadlift','Pistol Squat','Box Jump','Zittende Kuitheffen'],
  'Schouders': ['Overhead Press','Zijwaartse Heffing','Voorwaartse Heffing','Arnold Press','Rear Delt Fly','Upright Row','Schouderoptrekken','Pike Push-Ups','Handstand Push-Ups','Wall Walk','Dumbbell Shoulderpress','Kabel Zijwaartse Heffing','Push Press','Band Pull-Apart','Zittende Dumbbell Press','Plate Voorwaartse Heffing'],
  'Armen': ['Barbell Bicepscurl','Hamercurl','Schuine Halter Curl','Tricep Dip','Skull Crusher','Kabel Tricep Pushdown','Preacher Curl','Stoel Dips','Close-Grip Push-Ups','Concentration Curl','Kabel Curl','Overhead Tricep Extension','EZ-Bar Curl','Reverse Curl','Diamond Dips'],
  'Core': ['Plank','Crunches','Russian Twist','Hangende Beenheffing','Mountain Climbers','Bicycle Crunch','Zij-Plank','Ab Wheel Rollout','Flutter Kicks','V-Ups','Dead Bug','Hollow Hold','Beenheffing','Sit-Ups','Reverse Crunch','Pallof Press','Kabel Crunch','Woodchopper','Bird Dog'],
  'Cardio': ['Jumping Jacks','Burpees','High Knees','Touwtjespringen','Sprint Intervallen','Mountain Climbers','Roeimachine','Trapklimmer','Burpee Broad Jump','Skater Jumps','Bear Crawl','Butt Kicks','Battle Ropes','Box Step-Ups','Fietsen','Jumping Lunges','Tuck Jumps','Schaduwboksen'],
  'Billen': ['Barbell Hip Thrust','Cable Kickback','Frog Pump','Banded Lateral Walk','Donkey Kick','Fire Hydrant','Eenbenige Deadlift','Clamshell'],
  'Volledig Lichaam': ['Kettlebell Swing','Thruster','Turkish Get-Up',"Farmer's Carry",'Wall Ball','Clean and Press','Devil Press','Man Maker']
};
const EXERCISE_LIBRARY_AR = {
  'الصدر': ['ضغط البنش بالبار','ضغط الدمبل المائل','ضغط البنش المنحدر','تمرين الضغط (بوش أب)','فتح الكابل (فلاي)','الغطس (ديبس)','ضغط الصدر بالجهاز','جهاز بيك ديك','تمرين الضغط الماسي','تمرين الضغط العريض','تمرين الضغط المائل للأعلى','تمرين الضغط المائل للأسفل','تمرين الضغط بذراع واحدة (آرتشر)','ضغط البنش بالدمبل','ضغط مائل بالبار','تقاطع الكابل','بولأوفر بالدمبل','ضغط الأرضية','تمرين الضغط الانفجاري'],
  'الظهر': ['العقلة (بول أب)','سحب أمامي بالكابل','سحب البار وأنت منحني','تجديف بالكابل جالسًا','الرفعة الميتة','تجديف تي-بار','تجديف بدمبل بذراع واحدة','سحب الوجه (فيس بول)','سحب عقلي (تشين أب)','تجديف مقلوب','تمرين سوبرمان','ملائكة الثلج العكسية','عقلة بقبضة عريضة','تجديف بندلاي','سحب بذراع مستقيمة','سحب من الرف','تمرين الانحناء الأمامي (جود مورنينغ)','تمديد الظهر'],
  'الأرجل': ['سكوات بالبار','الطعنات أثناء المشي','ضغط الأرجل (ليغ برس)','الرفعة الميتة الرومانية','رفع الساق (كالف ريز)','تمديد الساق','ثني الساق','سكوات بلغاري منقسم','دفع الورك (هيب ثرست)','سكوات بوزن الجسم','قفزة سكوات','طعنة خلفية','الجلوس على الحائط','صعود المنصة','جسر الأرداف','جسر الأرداف بساق واحدة','رفع الساق بوزن الجسم','سكوات أمامي','سكوات جوبلت','الرفعة الميتة سومو','سكوات المسدس','القفز على الصندوق','رفع الساق جالسًا'],
  'الأكتاف': ['الضغط العلوي (أوفرهيد بريس)','رفع جانبي','رفع أمامي','ضغط أرنولد','فتح الكتف الخلفي','سحب مرتفع (أبرايت رو)','رفع الكتفين (شرَغ)','تمرين الضغط بايك','الضغط بوضعية الوقوف على اليدين','المشي على اليدين على الحائط','ضغط الكتف بالدمبل','رفع جانبي بالكابل','بوش بريس','فرد الحزام المطاطي','ضغط الدمبل جالسًا','رفع أمامي بالقرص'],
  'الذراعين': ['ثني العضلة الثنائية بالبار','ثني هامر','ثني الدمبل المائل','غطس الترايسبس','سكل كراشر','دفع الترايسبس بالكابل','ثني بريتشر','غطس الكرسي','الضغط بقبضة ضيقة','ثني تركيز','ثني بالكابل','تمديد الترايسبس فوق الرأس','ثني بار EZ','ثني عكسي','غطس الماس'],
  'الجذع': ['البلانك','تمرين الكرنش','اللف الروسي','رفع الساق معلقًا','متسلق الجبل','كرنش الدراجة','بلانك جانبي','عجلة البطن','ركلات الرفرفة','تمرين V','الحشرة الميتة','الثبات المجوف','رفع الساق','تمرين البطن (سيت أب)','كرنش عكسي','ضغط بالوف','كرنش بالكابل','تمرين قاطع الخشب','تمرين بيرد دوغ'],
  ...
  'الجسم بالكامل': ['أرجحة الكيتلبل','ثراستر','النهوض التركي','حمل المزارع','كرة الحائط','كلين آند بريس','ديفل بريس','مان ميكر']
};
...
function saveExSetsModal(){
  ...
  ex.mode = exSetsMode;
  ex.sets = parseInt(document.getElementById('exSetsInput').value) || 0;
...
function toggleProgramManage(){
  ...
  // Also reveals the per-exercise drag handle / how-to video / delete (✕)
  // controls, which stay hidden the rest of the time so the day view reads
  // clean until you actually want to reorder or edit exercises.
  const body = document.getElementById('workoutProgramBody');
  if(body) body.classList.toggle('pm-editing', programManageOpen);
}
...
async function renderProfile(){
  ...
  const ph = document.getElementById('ppPlaceholder');
  const topbarImg = document.getElementById('topbarAvatarImg');
  const topbarPh = document.getElementById('topbarAvatarPlaceholder');
  let avatarSrc = null;
  if(state.profile.photoId){
    avatarSrc = await fetchPhoto(state.profile.photoId);
  }
  const ppWrapEl = document.getElementById('ppWrap');
  if(avatarSrc){
    img.src = avatarSrc; img.style.display='block'; ph.style.display='none';
    topbarImg.src = avatarSrc; topbarImg.style.display='block'; topbarPh.style.display='none';
    if(ppWrapEl) ppWrapEl.classList.add('has-photo');
    applyProfilePhotoPosition();
  } else {
    img.style.display='none'; ph.style.display='block';
    topbarImg.style.display='none'; topbarPh.style.display='block';
    topbarPh.textContent = '＋';
    if(ppWrapEl) ppWrapEl.classList.remove('has-photo');
  }
...
// ── Profile photo: drag-to-reposition ──
function applyProfilePhotoPosition(){
  const px = (state.profile.photoPosX ?? 50), py = (state.profile.photoPosY ?? 50);
  const zoom = state.profile.photoZoom ?? 1;
  const posStr = `${px}% ${py}%`;
  const tf = `scaleX(${state.profile.photoFlip ? -1 : 1}) scale(${zoom})`;
  const img = document.getElementById('ppImg');
  const topbarImg = document.getElementById('topbarAvatarImg');
  if(img){ img.style.objectPosition = posStr; img.style.transform = tf; }
  if(topbarImg){ topbarImg.style.objectPosition = posStr; topbarImg.style.transform = tf; }
}
// Minimum zoom for the photo editor. At exactly "cover" scale (100%), an
// image's aspect ratio vs. the frame's aspect ratio means one axis fits with
// zero slack — so dragging only moves the photo in the other direction.
// Starting slightly zoomed in guarantees overflow (and so drag room) on BOTH
// axes, for any photo, so up/down and left/right repositioning always work.
const PE_MIN_ZOOM = 1.15;
...
// ═══════════════ Unified photo editor (profile / banner / before-after) ═══════════════
// One popup for add + resize (zoom) + reposition (drag) + flip, with a live
// preview, committed only on Save.
let peTarget = null;     // 'profile' | 'banner' | 'before' | 'after'
...
let peDraft = { dataUrl: null, isNewFile: false, posX: 50, posY: 50, zoom: PE_MIN_ZOOM, flip: false };

function peShapeFor(target){
  return target === 'profile' ? 'circle' : (target === 'banner' ? 'banner' : 'portrait');
}
async function openPhotoEditor(target, index){
  ...
  if(target === 'profile'){
    peOrigRef = state.profile.photoId || null;
    posX = state.profile.photoPosX ?? 50; posY = state.profile.photoPosY ?? 50;
    zoom = state.profile.photoZoom ?? 1; flip = !!state.profile.photoFlip;
  } else if(target === 'banner'){
    peOrigRef = state.profile.bannerBgPhotoId || null;
    posX = state.profile.bannerBgPosX ?? 50; posY = state.profile.bannerBgPosY ?? 50;
    zoom = state.profile.bannerBgZoom ?? 1; flip = !!state.profile.bannerBgFlip;
  } else {
  ...
  zoom = clampZoom(zoom);
  peDraft = { dataUrl: peOrigRef ? await fetchPhoto(peOrigRef) : null, isNewFile: false, posX, posY, zoom, flip };
  document.getElementById('peFrame').className = 'pe-frame pe-shape-' + peShapeFor(target);
  document.getElementById('peTitle').textContent = t(
    target === 'profile' ? 'pe_title_profile' : (target === 'banner' ? 'modal_bannerbg_title' : 'pe_title_progress')
  );
...
function peRenderPreview(){
  ...
  if(peDraft.dataUrl){
    img.src = peDraft.dataUrl;
    img.style.display = 'block';
    img.style.objectPosition = peDraft.posX + '% ' + peDraft.posY + '%';
    img.style.transform = `scaleX(${peDraft.flip ? -1 : 1}) scale(${peDraft.zoom})`;
  ...
  // The "Choose Image" button always stays visible/tappable, even before a photo
  // is picked, so adding a photo never depends solely on tapping the custom
  // drag/reposition frame (which some mobile browsers can swallow taps on).
  document.getElementById('peZoomPct').textContent = Math.round(peDraft.zoom*100)+'%';
...
function peStepZoom(delta){ peDraft.zoom = clampZoom(peDraft.zoom + delta); peRenderPreview(); }
function peResetZoom(){ peDraft.zoom = PE_MIN_ZOOM; peRenderPreview(); }
...
async function peSave(){
  ...
  try{
    ...
    if(peTarget === 'profile'){
      state.profile.photoId = ref;
      state.profile.photoPosX = peDraft.posX; state.profile.photoPosY = peDraft.posY;
      state.profile.photoZoom = peDraft.zoom; state.profile.photoFlip = peDraft.flip;
      await renderProfile();
    } else if(peTarget === 'banner'){
      state.profile.bannerBgPhotoId = ref;
      state.profile.bannerBgPosX = peDraft.posX; state.profile.bannerBgPosY = peDraft.posY;
      state.profile.bannerBgZoom = peDraft.zoom; state.profile.bannerBgFlip = peDraft.flip;
      await applyBannerBg();
      renderBannerBgControls();
    } else {
      if(peEditIndex != null){
        const item = state.photos[peTarget][peEditIndex];
        item.ref = ref; item.posX = peDraft.posX; item.posY = peDraft.posY; item.zoom = peDraft.zoom; item.flip = peDraft.flip;
      } else {
        state.photos[peTarget].push({ ref, date: todayISO(), posX: peDraft.posX, posY: peDraft.posY, zoom: peDraft.zoom, flip: peDraft.flip });
      }
      await renderProgressPhoto();
    }
    save();
...
async function peRemove(){
  ...
  if(peTarget === 'profile'){
    state.profile.photoId = null; state.profile.photo = null;
    state.profile.photoPosX = 50; state.profile.photoPosY = 50; state.profile.photoZoom = 1; state.profile.photoFlip = false;
    await renderProfile();
  } else if(peTarget === 'banner'){
    state.profile.bannerBgPhotoId = null;
    state.profile.bannerBgPosX = 50; state.profile.bannerBgPosY = 50; state.profile.bannerBgZoom = 1; state.profile.bannerBgFlip = false;
    await applyBannerBg();
    renderBannerBgControls();
  } else if(peEditIndex != null){
  ...
  peOrigRef = null;
  peEditIndex = null;
  peDraft = { dataUrl: null, isNewFile: false, posX: 50, posY: 50, zoom: PE_MIN_ZOOM, flip: false };
  peRenderPreview();
}
function initDragRepositioning(){
  const frame = document.getElementById('peFrame');
  if(!frame) return;
  setupDragReposition(frame, {
    isActive: () => !!peDraft.dataUrl,
    draggingClass: 'dragging',
    getPos: () => ({ x: peDraft.posX, y: peDraft.posY }),
    setPos: (x, y) => { peDraft.posX = x; peDraft.posY = y; peRenderPreview(); }
    // No onClick handler here: adding a photo is handled natively by the
    // <label for="peFileInput"> placeholder (#peEmpty), which is far more
    // reliable across mobile browsers than a JS-driven click.
  });
  frame.addEventListener('wheel', (e) => {
    if(!peDraft.dataUrl) return;
    e.preventDefault();
    peStepZoom(e.deltaY < 0 ? 0.05 : -0.05);
  }, { passive: false });
}
document.addEventListener('DOMContentLoaded', initDragRepositioning);
...
async function renderProgressPhoto(){
  ...
  if(item && src){
    ...
    const fl = item.flip ? -1 : 1;
    img.src = src;
    img.style.objectPosition = px + '% ' + py + '%';
    img.style.transform = `scaleX(${fl}) scale(${z})`;
    img.style.display = 'block';
    empty.style.display = 'none';
    frame.classList.add('has-photo');
  } else {
...
let lt = { phase:'idle', exercises:[], idx:0, set:1, totalSets:1, restSec:90, defaultSets:4, remaining:0, running:false, warmupSteps:[], warmupStepIdx:0, circuitMode:false, sequence:[], seqPos:0 };
let ltInterval = null;

// Builds the ordered list of {idx,set} steps the Voice Coach walks through.
// Straight mode: all sets of exercise 1, then all sets of exercise 2, etc.
// Circuit mode: set 1 of every exercise (one round), then set 2 of every exercise, etc.
// Exercises with fewer sets than the round simply get skipped in later rounds.
function ltBuildSequence(exercises, circuitMode, defaultSets){
  const seq = [];
  if(circuitMode){
    const maxSets = exercises.reduce((m,ex)=>Math.max(m, ex.sets || defaultSets), 1);
    for(let s=1; s<=maxSets; s++){
      exercises.forEach((ex,i)=>{
        const total = ex.sets || defaultSets;
        if(s<=total) seq.push({idx:i, set:s});
      });
    }
  } else {
    exercises.forEach((ex,i)=>{
...
function ltStart(){
  ...
  if(lt.phase==='idle' || lt.phase==='done'){
    ...
    lt.sequence = ltBuildSequence(lt.exercises, lt.circuitMode, lt.defaultSets);
    lt.seqPos = 0;
    lt.idx = lt.sequence[0].idx; lt.set = lt.sequence[0].set;
    ...
    const steps = chosenWarmup ? warmupSteps(chosenWarmup) : [];
    if(steps.length){
      lt.phase = 'warmup';
      lt.warmupSteps = steps;
      lt.warmupStepIdx = 0;
      ltStartWarmupStep();
    } else {
      ltAnnounceSet();
...
function ltAnnounceSet(){
  ...
  lt.phase = 'lift';
  clearInterval(ltInterval);
  const setPart = lt.totalSets > 1 ? ('. ' + tphrase('set_of')(lt.set, lt.totalSets)) : '';
  if(ex.mode === 'time' && ex.holdSec){
    lt.remaining = ex.holdSec;
    vcSpeak(ex.name + '. ' + tphrase('hold_for')(ex.holdSec) + setPart);
    ltInterval = setInterval(ltTick, 1000);
  } else {
    const repsPart = ex.reps ? (', ' + ex.reps + ' ' + tphrase('reps_word')) : '';
    vcSpeak(ex.name + repsPart + setPart);
  }
}
function ltDone(){
...
function ltTick(){
  ...
  if(lt.phase === 'rest' || lt.phase === 'warmup'){
...
function ltNext(){
  ...
  lt.idx = step.idx; lt.set = step.set;
  ltAnnounceSet();
  ltRender();
}
function ltSkip(){
  if(lt.phase === 'warmup'){ clearInterval(ltInterval); ltAdvanceWarmup(); }
...
function ltReset(){
  ...
  lt = { phase:'idle', exercises:[], idx:0, set:1, totalSets:1, restSec:90, defaultSets:4, remaining:0, running:false, warmupSteps:[], warmupStepIdx:0, circuitMode:false, sequence:[], seqPos:0 };
...
function ltRender(){
  ...
  phaseEl.style.color = color;
  timeEl.style.color = color;
  timeEl.textContent = (lt.phase === 'rest' || isTimedLift || lt.phase === 'warmup') ? ln(lt.remaining) : (lt.phase === 'lift' ? '💪' : '--');
  ...
  if(lt.phase === 'idle'){
    ...
    const setBit = lt.totalSets > 1 ? ` · ${t('st_set')} ${ln(lt.set)} / ${ln(lt.totalSets)}` : '';
    setLbl.textContent = `${t('st_exercise')} ${ln(lt.idx+1)} / ${ln(lt.exercises.length)}${setBit}`;
    if(lt.phase === 'rest'){
      const nextEx = (lt.set >= lt.totalSets) ? lt.exercises[lt.idx+1] : lt.exercises[lt.idx];
...
function wireStaticHandlers(){
  ...
  bind('h7','click',function(event){ openProfileFromAvatar() });
  ...
  bind('h16','click',function(event){ openPhotoEditor('profile') });
  bind('h17','click',function(event){ toggleBannerEditDropdown() });
  bind('h18','click',function(event){ closeBannerEditModal() });
  bind('h19','click',function(event){ toggleSettingsGroup(this) });
  bind('h20','click',function(event){ setBannerBgType('default') });
  bind('h21','click',function(event){ setBannerBgType('solid') });
  bind('h22','click',function(event){ setBannerBgType('image') });
  bind('h23','change',function(event){ setBannerBgColor(this.value) });
  bind('h24','click',function(event){ openPhotoEditor('banner') });
...
const DYNAMIC_CLICK_ACTIONS = {
  ...
  openExerciseVideoSearch: function(el){ openExerciseVideoSearch(parseInt(el.dataset.idx, 10)); },
